...
class PRUSafePretrain:
    ...
    def _learn_Qc(self, loader):
        qc_logger = Logger(exp_name='Qc', data_dir=self.logger.log_dir, seed=self.cfgs.seed)
        qc_logger.setup_torch_saver({'Qc': self.cost_critic})
        self.vae_copy = deepcopy(self.vae)
        self.vae_copy_optimizer = set_optimizer(
            'Adam', module=self.vae_copy, learning_rate=self.cfgs.vae_lr
        )
        qc_logger.log('Start learning Qc...')

        for epoch in range(self.cfgs.Qc_epochs):
            self.epoch_step = epoch
            self.epoch_start_time = time.time()

            # train
            for grad_step, batch in enumerate(loader):
                self._train_Qc(grad_step, batch, qc_logger)
                if grad_step >= self.cfgs.grad_steps_per_epoch:
                    break

            # log
            self._qc_log(qc_logger)

            if (epoch + 1) % self.cfgs.save_freq == 0:
                qc_logger.torch_save(itr=epoch)

        qc_logger.log('Finish learning Qc...')
        qc_logger.close()

    def _train_Qc(self, grad_step, batch, logger):
        obs, act, reward, cost, done, next_obs = batch

        act_next = self.vae_copy.decode(next_obs)

        with torch.no_grad():
            qc1_next, qc2_next = self.target_cost_critic(next_obs, act_next)
            qc_next = torch.min(qc1_next, qc2_next)
            target_qc = cost + (1 - done) * qc_next

        qc1, qc2 = self.cost_critic(obs, act)

        qc_loss = nn.functional.mse_loss(qc1, target_qc) + nn.functional.mse_loss(qc2, target_qc)

        self.cost_critic_optimizer.zero_grad()
        qc_loss.backward()
        self.cost_critic_optimizer.step()

        kl_loss, recon_loss = self.vae_copy.loss(obs, act)
        self.vae_copy_optimizer.zero_grad()
        (kl_loss + recon_loss).backward()
        self.vae_copy_optimizer.step()

        self._soft_update_cost_target()
        ...
        if grad_step == self.cfgs.grad_steps_per_epoch - 1:
            logger.store(
                **{
                    ...
                    'Qc/target_Qc': target_qc[0].mean().item(),
                    'Qc/data_Qc': qc1[0].mean().item(),

                    'Loss/cost_critic_loss': qc_loss.item(),
                    'Loss/kl_loss': kl_loss.item(),
                    'Loss/recon_loss': recon_loss.item(),
                }
            )

    def _qc_log(self, logger):
        logger.log_tabular('Epoch')
        logger.log_tabular('GradStep')
        logger.log_tabular('Time/Epoch')
        logger.log_tabular('Time/Total')

        logger.log_tabular('Qc/target_Qc')
        logger.log_tabular('Qc/data_Qc')

        logger.log_tabular('Loss/cost_critic_loss')
        logger.log_tabular('Loss/kl_loss')
        logger.log_tabular('Loss/recon_loss')
    ...
    def _train(self, grad_step, batch):
        ...
        qr_target = (reward + (1 - done) * self.cfgs.gamma * (qr_next - beta_in * uncertainty_next)) * is_safe + \
            torch.min(qr - self.cfgs.cost_scale * beta_out * uncertainty, torch.zeros_like(qr)) * (1 - is_safe)
    ...
    def _soft_update_cost_target(self):
