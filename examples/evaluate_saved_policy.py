...
for algo in os.scandir('./runs/SafetyPointGoal1-v0'):
    for exp in os.scandir(algo):
        for item in os.scandir(os.path.join(exp, 'torch_save')):
            if item.is_file() and item.name.split('.')[-1] == 'pt':
                evaluator.load_saved_model(save_dir=exp, model_name=item.name)
                evaluator.render(num_episode=20, camera_name='track', width=256, height=256)
